...
Ma12070p<I2cMaster>::setLimiterThreshold(quarter_decibel_t ch0l, quarter_decibel_t ch0r, quarter_decibel_t ch1l, quarter_decibel_t ch1r)
{
	...
	tx_buffer[1] = qdbToRegValue(ch0l) >> 2;
	tx_buffer[2] = qdbToRegValue(ch0r) >> 2;
	tx_buffer[3] = qdbToRegValue(ch1l) >> 2;
	tx_buffer[4] = qdbToRegValue(ch1r) >> 2;
	tx_buffer[5] = (qdbToRegValue(ch0l) & 0b11) | ((qdbToRegValue(ch0r) & 0b11) << 2) | ((qdbToRegValue(ch1l) & 0b11) << 4) | ((qdbToRegValue(ch1r) & 0b11) << 6);
...
Ma12070p<I2cMaster>::setMasterVolume(quarter_decibel_t volume)
{
	...
	tx_buffer[1] = qdbToRegValue(volume) >> 2;
	tx_buffer[2] = (qdbToRegValue(volume) & 0b11);
	this->transaction.configureWrite(tx_buffer, 3);
	RF_END_RETURN_CALL(this->runTransaction());
}
