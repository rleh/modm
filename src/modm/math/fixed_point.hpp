...
class fixed_point_t
{
	...
	using WideType = typename modm::ArithmeticTraits<U>::WideType;
...
template<typename OU, uint8_t OF>
modm::IOStream&
operator<<(modm::IOStream& os, const modm::fixed_point_t<OU, OF>& fp) {
	return os << (fp.value() >> OF) << "." << (fp.value() & ((1 << (OF)) - 1));
}
